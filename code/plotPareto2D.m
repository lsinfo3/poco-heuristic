...
function [paretoidx textXaxis textYaxis]=plotPareto2D(xin,yin,varargin)
...
if xlims(2)>max(xinthin) || xlims(2)<max(xinthin)
    xlims(2)=max(xinthin)*1.2;
end

if yinthin~=0
    if (ylims(2)>2*max(yinthin) || ylims(2)<max(yinthin)) &&  ~isempty(yinthin)
...
if xlims(1)>min(xinthin) || xlims(1)<min(xinthin)
...
if ~isempty(p.Results.XLim)
    xlim([xlims(1) p.Results.XLim]);
else
    xlim(xlims);
end
if ~isempty(p.Results.YLim)
    ylim([ylims(1) p.Results.YLim]);
else
    ylim(ylims);
end
