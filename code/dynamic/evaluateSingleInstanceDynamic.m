...
function evaluationResult=evaluateSingleInstance(distanceMatrix,numberOfControllers,diameter,nodeWeights,workingsubset,singlePlacementInstance)
...
if ~exist('singlePlacementInstance','var')
    ...
    for i=1:numberOfControllers
        if nchoosek(size(distanceMatrix,2),i)*i<mylimit
            if ~exist(['variables/nk' num2str(size(distanceMatrix,2)) '_' num2str(i) '.mat'],'file') && i*nchoosek(size(distanceMatrix,2),i)<mylimit
                nk=combnk(1:size(distanceMatrix,2),i);
                save(['variables/nk' num2str(size(distanceMatrix,2)) '_' num2str(i) '.mat'],'nk');
            end
        else
            break;
        end
    ...
    nksize=nchoosek(size(distanceMatrix,2),numberOfControllers);
    
    if numberOfControllers*nksize<mylimit
        load(['variables/nk' num2str(size(distanceMatrix,2)) '_' num2str(numberOfControllers) '.mat']);
        ...
        for i=numberOfControllers:-1:1
            if i*nchoosek(size(distanceMatrix,2),i)<mylimit
        ...
        lstruct=load(['variables/nk' num2str(size(distanceMatrix,2)) '_' num2str(leftNumberOfControllers) '.mat']);
        left=lstruct.(char(fieldnames(lstruct)));
        lstruct=load(['variables/nk' num2str(size(distanceMatrix,2)) '_' num2str(rightNumberOfControllers) '.mat']);
