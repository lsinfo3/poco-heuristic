...
function plotBarsPLC(timeArray,tmvals,contNames,latencyvals,changes,imbalance,mycolors,figureid,axesid1,axesid2,axesid3,axesid4,axesid5,meanlatencyvals,avgLatency,avgTM,changesPLC,sumbalancearrayPLC,myidx,newPLCCalc,bestmaxidx,currentmaxidx,controllerIds)
cla(axesid2,'reset')
...
%     rectangle('Position',[0.9 1 length(latencyvals)+0.5 100],'FaceColor','w','EdgeColor','none')
    for i = 1:length(latencyvals)
        bar(i, ml(i),'FaceColor', getgreenyellowred(latencyvals(i)),'LineWidth',2);
        if isempty(find(controllerIds==i,1)) & meanlatencyvals(i)<inf
            plot(i+[-0.4 +0.4], [1 1]*meanml(i),'k+-','LineWidth',2);
        end
        idx=find(controllerIds==i,1);
        if ~isempty(idx)
            %             text('Position',[i,15],'String','C','VerticalAlignment','bottom','HorizontalAlignment','center','Color','k','FontSize',9);
            plot(i, 50,'o','LineWidth',1,'MarkerSize',7,'MarkerFaceColor',mycolors(idx,:),'Color',darken(mycolors(idx,:)));
            plot(i, 50,'o','LineWidth',1,'MarkerSize',9,'Color',darken(mycolors(idx,:)));
            plot(i, 50,'o','LineWidth',1,'MarkerSize',11,'Color',darken(mycolors(idx,:)));
        elseif ~isnan(meanlatencyvals(i)) & meanlatencyvals(i)<inf & meanlatencyvals(i)<0.6
        ...
        elseif isnan(meanlatencyvals(i))
            %             text('Position',[i, 15],'String',sprintf('cont\nless'),'VerticalAlignment','bottom','HorizontalAlignment','center','Color','k','FontSize',9);
            plot(i, 50,'o','MarkerFaceColor',[0.95 0.95 0.95],'MarkerSize',9,'LineWidth',1,'MarkerEdgeColor',[0.6 0.6 0.6]);
            text(i, 50,'?','FontSize',6,'FontWeight','bold','HorizontalAlignment','center','VerticalAlignment','middle','Color','r');
        elseif meanlatencyvals(i)>=0.6 & meanlatencyvals(i)<inf
            text('Position',[i, 600],'String','>600','VerticalAlignment','bottom','HorizontalAlignment','center','Color','k','FontSize',9);
        else
            %             text('Position',[i, 15],'String','broken','VerticalAlignment','bottom','HorizontalAlignment','center','Color','k','FontSize',9);
            plot(i, 50,'or','LineWidth',3,'MarkerSize',9,'MarkerFaceColor','w');
        end
    end
end
