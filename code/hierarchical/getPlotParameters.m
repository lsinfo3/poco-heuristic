...
if strcmpi(evaluationResult.failureType,'Node')
    [temp,worst_number_of_failures]=max(evaluationResult.(['failurePatternIndexOf' strrep(strrep(strrep(fieldname,'AllNodeFailures', ''), 'controller', 'Controller'), 'max', 'Max') 'ForNFailures'])(:,idx));
    clear temp;
    ...
    if ~isnan(evaluationResult.failurePatternIndexOfMaxLatencyN2CForNFailures(worst_number_of_failures,idx))
        nodesfailed=failurepatterns(evaluationResult.(['failurePatternIndexOf' strrep(strrep(strrep(fieldname,'AllNodeFailures', ''), 'controller', 'Controller'), 'max', 'Max') 'ForNFailures'])(worst_number_of_failures,idx),:);
    end  
...
elseif strcmpi(evaluationResult.failureType,'Controller')
    ...
    if matlabVersion < 2013
        controllersfailed=nk(idx,setdiff(1:size(nk,2),failurepatterns(evaluationResult.(['failurePatternIndexOf' strrep(strrep(strrep(strrep(fieldname,'AllControllerFailures', ''), 'controller', 'Controller'), 'max', 'Max'), 'Controllerless', 'MaxNumberOfControllerless') 'ForNFailures'])(worst_number_of_failures,idx),:)));
    else
        controllersfailed=nk(idx,setdiff(1:size(nk,2),failurepatterns(evaluationResult.(['failurePatternIndexOf' strrep(strrep(strrep(strrep(fieldname,'AllControllerFailures', ''), 'controller', 'Controller'), 'max', 'Max'), 'Controllerless', 'MaxNumberOfControllerless') 'ForNFailures'])(worst_number_of_failures,idx),:), 'legacy'));
    end
end